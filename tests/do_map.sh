...
mkdir -p ./res_map
start_std=$(date +%s.%N)
g++ -g -Wall -Werror -Wextra -std=c++98 -D NS=std map_tests.cpp > ./res_map/std_map.txt
end_std=$(date +%s.%N)
start_ft=$(date +%s.%N)
g++ -g -Wall -Werror -Wextra -std=c++98 -D NS=ft map_tests.cpp > ./res_map/ft_map.txt
end_ft=$(date +%s.%N)
...
max_allowed_time_ft=$(printf "%.0f" ${max_allowed_time_ft})
echo "${time_std}, ${time_ft}, ${max_allowed_time_ft}"
if [ max_allowed_time -le time_ft ]
then
